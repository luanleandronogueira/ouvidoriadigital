...
if(isset($_SERVER['REQUEST_METHOD']) && $_SERVER['REQUEST_METHOD'] === 'POST') {
    if(!empty($_POST)){       
        ...
        // Insere r no banco de dados a manifestacao e recupera seu ID
        $ultimo_id = $manifestacoes->inserir_manifestacao($motivo_manifestacao, $id_entidade_manifestacao, $id_tipo_manifestacao, $conteudo_manifestacao, $observacoes_manifestacao, $local_manifestacao, $arquivo_manifestacao, $id_usuario_manifestacao, 'A', $data_manifestacao, $protocolo_manifestacao);

        // lógica para salvar no banco a localização com ID, protocolo 

        // Coleta o IP da REQUISIÇÃO
        $ip = coleta_ip();

        // inserir no banco a a localização
        $localizacao->inserir_localizacao($ultimo_id, $_POST['latitude'], $_POST['longitude'], $ip);
        ...
        try {
            ...
            //Recipients
            $mail->setFrom('naoresponda@it-solucoes.inf.br', 'Nova Solicitação na Ouvidoria');
